...
export default function KnockoutStage() {
  ...
  const { bracket, knockoutRound, playNextKnockoutRound, isSimulating } = useGame();
    ...
    return (
      <div className="mb-10">
        ...
        <div className={`grid gap-3 ${
          matchArray.length > 4 ? 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-4' :
          matchArray.length > 2 ? 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-4' :
          matchArray.length === 2 ? 'grid-cols-1 sm:grid-cols-2' :
        ...
        <div className="flex justify-center mb-8">
          ...
          >
            {isSimulating ? (
              ...
              <><Play className="w-5 h-5 mr-2" />{t('playNextRound')}: {currentLabel}</>
            )}
          </Button>
        </div>
